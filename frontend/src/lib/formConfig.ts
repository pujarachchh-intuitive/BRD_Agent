// Declarative description of the request form. Mirrors webapp/service.py's FORM_FIELDS exactly
// (field name -> kind), plus the display metadata (labels, helper text, sub-fields) needed to
// render it. Ported 1:1 from webapp/static/form-config.js — this is the single source of truth
// the form is built from.

import { FieldDef, FormSection } from "./types";
...
export const FIELD_DEFS: Record<string, FieldDef> = {
  ...
  functional_requirements: {
    ...
    helper:
      "M = Must have, S = Should have, C = Could have. Auto-generate/enrich will keep every requirement you list and add any more it thinks the solution needs.",
    subfields: [
  ...
  system_components: {
    ...
    subfields: [
      ...
      { name: "responsibility", label: "Responsibility", type: "text" },
      {
        ...
        type: "text",
        placeholder: "comma-separated, e.g. API, Database",
        isList: true,
      },
    ],
    noExclude: true,
